...
function ProfileContent() {
  ...
  const [activeTab, setActiveTab] = useState<"rezerwacje" | "ulubione">("rezerwacje");
      ...
      <main className="flex-1 overflow-y-auto px-5">
        ...
        <div className="flex flex-col items-center pt-4 pb-5">
          ...
          >
            ...
            <span className="text-xs text-muted dark:text-muted-foreground/70">{user.email}</span>
          </motion.div>
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            ...
            <span className="text-[10px] font-semibold text-emerald-500">Konto zweryfikowane</span>
          </motion.div>
        </div>
        ...
        >
          <div className="flex-1 px-4 py-3 rounded-2xl bg-black/3 dark:bg-white/5 border border-border/60 text-center">
            <span className="text-lg font-extrabold text-foreground">{bookings.length}</span>
            <p className="text-[9px] text-muted dark:text-muted-foreground/70 mt-0.5">Rezerwacje</p>
          </div>
          <div className="flex-1 px-4 py-3 rounded-2xl bg-black/3 dark:bg-white/5 border border-border/60 text-center">
            <span className="text-lg font-extrabold text-foreground">{favorites.length}</span>
            <p className="text-[9px] text-muted dark:text-muted-foreground/70 mt-0.5">Ulubione</p>
          </div>
        </motion.div>
      ...
      {bookings.map((b, i) => (
        ...
        >
          ...
          <div className="relative w-14 h-14 rounded-xl overflow-hidden shrink-0">
            <Image src={b.villaImage} alt={b.villaName} fill className="object-cover" sizes="56px" />
          </div>

          {/* Info */}
          <div className="flex-1 min-w-0">
            <p className="text-xs font-semibold text-foreground truncate">{b.villaName}</p>
            <div className="flex items-center gap-1 mt-1">
              <Calendar className="w-3 h-3 text-foreground/40" />
              <span className="text-[10px] text-muted dark:text-muted-foreground/70">
                {new Date(b.checkIn).toLocaleDateString("pl-PL", { day: "numeric", month: "short" })}
                {" – "}
                {new Date(b.checkOut).toLocaleDateString("pl-PL", { day: "numeric", month: "short" })}
              </span>
            </div>
            <div className="flex items-center justify-between mt-1">
              <span className="text-[10px] text-muted dark:text-muted-foreground/70">
                {b.nightsCount} {b.nightsCount === 1 ? "noc" : "noce"} · {b.guests} {b.guests === 1 ? "gość" : "gości"}
              </span>
              <span className="text-[11px] font-bold text-foreground">{b.totalPrice} zł</span>
            </div>
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className="flex items-center gap-0.5">
                ...
                <span className="text-[10px] font-bold text-foreground">{v.rating}</span>
              </div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[11px] font-extrabold text-foreground">{v.price} zł</span>
              <span className="text-[9px] text-muted dark:text-muted-foreground/70">/ noc</span>
              <span className="text-[9px] text-muted dark:text-muted-foreground/70">·</span>
              <span className="text-[9px] text-muted dark:text-muted-foreground/70">{v.distanceToBeach} m od plaży</span>
            </div>
          </div>
    ...
    <>
      ...
      <div className="hidden md:flex min-h-screen w-full items-center justify-center relative overflow-hidden bg-gradient-to-br from-[#0a2540] via-[#1a3a5c] to-[#0a2540]">
        ...
        <div className="absolute top-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1">
          <span className="text-white/40 text-xs tracking-[0.3em] uppercase font-medium">Ville Kołobrzeg</span>
          <span className="text-white/20 text-[10px] tracking-wider">Mobilna platforma rezerwacji willi</span>
        </div>

        <div className="relative" style={{ width: "390px", height: "844px" }}>
          <div className="absolute inset-0 rounded-[55px] shadow-[0_60px_120px_rgba(0,0,0,0.6),0_0_0_1px_rgba(255,255,255,0.08)] overflow-hidden"
            style={{ background: "linear-gradient(145deg, #2a2a2a 0%, #1a1a1a 30%, #0d0d0d 60%, #1a1a1a 100%)" }}
          >
            <div className="absolute rounded-[48px] overflow-hidden bg-background" style={{ inset: "8px" }}>
              ...
              <div className="absolute inset-0 pt-[34px]"><ProfileContent /></div>
            </div>
            <div className="absolute rounded-r-sm bg-zinc-700" style={{ right: "-3px", top: "160px", width: "3px", height: "60px" }} />
            <div className="absolute rounded-l-sm bg-zinc-700" style={{ left: "-3px", top: "140px", width: "3px", height: "40px" }} />
            <div className="absolute rounded-l-sm bg-zinc-700" style={{ left: "-3px", top: "195px", width: "3px", height: "40px" }} />
            <div className="absolute rounded-l-sm bg-zinc-700" style={{ left: "-3px", top: "100px", width: "3px", height: "28px" }} />
          </div>
          <div className="absolute pointer-events-none rounded-[48px] opacity-20"
            style={{ inset: "8px", background: "linear-gradient(120deg, rgba(255,255,255,0.15) 0%, rgba(255,255,255,0) 50%)" }}
          />
        </div>
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
          <span className="text-white/20 text-[10px] tracking-widest uppercase">© 2026 Ville Kołobrzeg</span>
        </div>
      </div>
    </>
  );
