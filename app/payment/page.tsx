...
const paymentMethods: {
  ...
  { id: "card", name: "Karta płatnicza", icon: CreditCard, description: "Visa, Mastercard" },
  { id: "blik", name: "BLIK", icon: Smartphone, description: "Szybki przelew mobilny" },
  { id: "apple", name: "Apple Pay", icon: Wallet, description: "Zapłać szybko i bezpiecznie" },
  { id: "google", name: "Google Pay", icon: Wallet, description: "Twoje karty w jednym miejscu" },
  { id: "przelewy24", name: "Przelewy24", icon: Banknote, description: "Przelew online" },
]
...
function PaymentContent() {
  ...
  const [booking] = useState<BookingData | null>(() => {
    ...
    return {
      ...
      villaImage: searchParams?.get("villaImage") || "/images/willa_bursztynowa.png",
            ...
            <div className="flex flex-col items-center gap-1.5">
              ...
              >
                {isActive && i < stepIndex ? (
                  ...
                  <span className={`text-sm font-bold ${isActive ? "text-accent-foreground" : ""}`}>
                    {step.number}
                  </span>
                )}
              </motion.div>
              <span
                className={`text-[9px] font-semibold tracking-tight ${
                  isActive ? "text-foreground" : "text-muted dark:text-muted-foreground/50"
                }`}
              >
                {step.label}
              </span>
  ...
  const renderSummary = () => (
    ...
    >
      ...
      <div className="bg-black/3 dark:bg-white/5 rounded-2xl p-4 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted dark:text-muted-foreground/70">Czyszczenie</span>
          <span className="text-xs font-medium text-foreground">150 zł</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-xs text-muted dark:text-muted-foreground/70">Opłata serwisowa</span>
      ...
      <div className="flex items-start gap-2.5 px-3 py-3 rounded-2xl bg-yellow-400/5 border border-yellow-400/20">
        ...
        <p className="text-[11px] text-muted dark:text-muted-foreground/70 leading-relaxed">
          Nie ponosisz opłat do momentu potwierdzenia rezerwacji. Płatność zostanie
          przetworzona w bezpiecznym środowisku.
        </p>
      </div>
    ...
    >
      <div className="bg-black/3 dark:bg-white/5 rounded-2xl p-3 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-xs font-semibold text-foreground">Do zapłaty</span>
      ...
      <div className="space-y-3">
        ...
        <button
          ...
          disabled={
            ...
            processing ||
            (selectedMethod === "card" &&
              (!cardNumber || cardNumber.replace(/\s/g, "").length < 16 || !cardExpiry || !cardCvc || !cardName)) ||
  ...
  const renderConfirmation = () => (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <span className="text-[11px] text-muted dark:text-muted-foreground/70">Metoda</span>
          <span className="text-[11px] font-semibold text-foreground">
            {paymentMethods.find((m) => m.id === selectedMethod)?.name || "Karta"}
          </span>
        </div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-sm font-bold text-foreground">Regulamin i polityka</h2>
                <button
              ...
              <div className="space-y-3 text-[11px] text-muted dark:text-muted-foreground/70 leading-relaxed max-h-64 overflow-y-auto">
                <p>
                  Rezerwacja jest wiążąca po dokonaniu pełnej płatności. Anulowanie
                  rezerwacji na 48h przed przyjazdem jest bezpłatne.
                </p>
                <p>
                  W przypadku anulowania w terminie krótszym niż 48h, pobierana jest
                  opłata w wysokości 50% wartości rezerwacji.
                </p>
                <p>
                ...
                <p>
                  ...
                  płatności. Dane karty nie są przechowywane na naszych serwerach.
                </p>
              </div>
              <button
                onClick={() => setShowRegulations(false)}
